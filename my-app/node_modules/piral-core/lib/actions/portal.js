"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.showPortal = exports.updatePortal = exports.hidePortal = exports.destroyPortal = void 0;
const utils_1 = require("../utils");
function destroyPortal(ctx, id) {
    ctx.dispatch((state) => ({
        ...state,
        portals: (0, utils_1.withoutKey)(state.portals, id),
    }));
}
exports.destroyPortal = destroyPortal;
function hidePortal(ctx, id, entry) {
    ctx.dispatch((state) => ({
        ...state,
        portals: (0, utils_1.withKey)(state.portals, id, (0, utils_1.excludeItem)(state.portals[id], entry)),
    }));
}
exports.hidePortal = hidePortal;
function updatePortal(ctx, id, current, next) {
    ctx.dispatch((state) => ({
        ...state,
        portals: (0, utils_1.withKey)(state.portals, id, (0, utils_1.replaceOrAddItem)(state.portals[id], next, (m) => m === current)),
    }));
}
exports.updatePortal = updatePortal;
function showPortal(ctx, id, entry) {
    ctx.dispatch((state) => ({
        ...state,
        portals: (0, utils_1.withKey)(state.portals, id, (0, utils_1.includeItem)(state.portals[id], entry)),
    }));
}
exports.showPortal = showPortal;
//# sourceMappingURL=portal.js.map