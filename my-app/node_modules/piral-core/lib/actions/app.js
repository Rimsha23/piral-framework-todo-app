...
function removePilet(ctx, name) {
    ctx.dispatch((state) => ({
        ...
        registry: (0, utils_1.removeNested)(state.registry, (m) => m.pilet === name),
    }));
...
function injectPilet(ctx, pilet) {
    ctx.dispatch((state) => ({
        ...state,
        modules: (0, utils_1.replaceOrAddItem)(state.modules, pilet, (m) => m.name === pilet.name),
        registry: (0, utils_1.removeNested)(state.registry, (m) => m.pilet === pilet.name),
    }));
    ctx.emit('unload-pilet', {
        name: pilet.name,
    });
...
function setComponent(ctx, name, component) {
    ctx.dispatch((state) => ({
        ...state,
        components: (0, utils_1.withKey)(state.components, name, component),
    }));
}
...
function setErrorComponent(ctx, type, component) {
    ctx.dispatch((state) => ({
        ...state,
        errorComponents: (0, utils_1.withKey)(state.errorComponents, type, component),
    }));
}
