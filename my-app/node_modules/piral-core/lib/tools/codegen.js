...
function getModulePathOrDefault(root, origin, name) {
    try {
        const absPath = (0, resolve_1.getModulePath)(root, name);
        const relPath = (0, path_1.relative)(origin, absPath);
        // The relative path is to be used in an import statement,
        // so it should be normalized back to use posix path separators.
        const path = relPath.split(path_1.sep).join(path_1.posix.sep);
        return path;
    }
    catch {
        return name;
