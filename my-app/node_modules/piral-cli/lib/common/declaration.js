...
async function findPiralInstanceApi(piralInstance) {
    if (piralInstance) {
        const path = require.resolve(`${piralInstance}/${constants_1.packageJson}`);
        const root = (0, path_1.dirname)(path);
        const data = await (0, io_1.readJson)(root, constants_1.packageJson);
        const subpath = data.types || data.typings;
        if (subpath) {
            return [
                {
                    file: (0, path_1.resolve)(root, subpath),
                    name: piletApiName,
                },
            ];
        }
...
async function createDeclarationFile(options, source, target, forceOverwrite) {
...
async function createPiralDeclaration(baseDir, entry, target, forceOverwrite, logLevel) {
    ...
    const { name, root, externals, framework } = await (0, package_1.retrievePiletsInfo)(entryFiles);
