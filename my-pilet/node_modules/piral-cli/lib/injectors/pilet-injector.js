...
async function loadFeed(feed) {
    try {
        const response = await external_1.axios.default.get(feed);
...
class PiletInjector {
    constructor(config, serverConfig, core) {
        ...
        if (this.config.active) {
            ...
            pilets.forEach((p, i) => p.bundler.on(async () => {
    ...
    async sendResponse(path, url, baseUrl) {
    ...
    download(path) {
        const url = new url_1.URL(path, this.proxyInfo.source);
        const auth = config_1.config.auth?.[this.proxyInfo.source];
        switch (auth?.mode) {
            case 'header':
                return external_1.axios.default.get(url.href, {
            ...
            default:
                return external_1.axios.default.get(url.href, { responseType: 'arraybuffer' });
        }
    }
